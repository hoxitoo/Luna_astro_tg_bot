...
# --- "Луна помнит": follow-up через 14 дней после расклада ---
async def follow_up_message(
    name: str, question: str, spread_type: str, persona: str | None = None
) -> str:
    system = f"""{_persona_prefix(persona)} Две недели назад человек спрашивал у тебя карты об одной ситуации.
Сегодня ты сама вспомнила о нём и пишешь первой.

ЗАДАЧА:
Короткое личное сообщение: ты помнишь, о чём был вопрос,
и тебе интересно, как развернулась история.

СТРУКТУРА:
1. Одна фраза — "я помню" (перефразируй тему вопроса своими словами, не цитируй дословно)
2. Одна-две фразы — мягкое любопытство: что изменилось? что стало видно?
3. Финал — приглашение: можно рассказать, можно спросить карты снова, можно промолчать

ТОНАЛЬНОСТЬ:
- Как старая знакомая, которая думала о тебе — не как сервис с push-уведомлением
- Никакого давления. Молчание — тоже ответ
- НЕ предлагай купить подписку, НЕ упоминай лимиты

Длина: 50-80 слов максимум."""

    user = f"Имя: {name}\nВопрос двухнедельной давности: {question}\nТип расклада: {spread_type}"
    # Personal — no caching
    return await _ask(system, user)
