"""ЮKassa (YooKassa) integration — KEPT FOR THE FUTURE, NOT wired into the
active paywall. The bot currently accepts Telegram Stars and Robokassa.

To revive YooKassa later:
  1. Add CRUD glue back (set_payment_provider_id / get_payment_by_provider_id /
     a provider-id variant of mark_payment_paid) — see git history of crud.py.
  2. Add a webhook handler (POST /yookassa/webhook) that re-fetches the payment
     via `fetch_payment` and grants on status == "succeeded".
  3. Add paywall buttons that call `create_payment_link`.
...
YooKassa has no request signatures. A webhook notification is therefore verified
by re-fetching the payment via the API (`fetch_payment`) and trusting only the
server's own copy of status/amount — never the values in the webhook body.
"""
import base64
import uuid
import aiohttp
